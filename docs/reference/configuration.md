...
| `agents.defaults.mascotMood` | bool | `false` | **Off by default for now** (the expression artwork is not drawn yet; the WebUI keeps its own matching switch off too, so enabling this alone costs a request per turn and shows nothing). When on: after each WebUI turn, a small background request asks the model how Jenny feels about the reply she just gave (one letter, a few hundred input tokens), and the mascot pulls the matching face for a few seconds. Nothing is added to the conversation or to the agent's prompt; turns that end in an error, command turns and very short replies cost nothing. Read per turn: a change applies to the next turn without a restart. |
