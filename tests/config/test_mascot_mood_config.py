...
def test_mascot_mood_accepts_camel_and_snake_aliases():
    camel = Config.model_validate({
        "agents": {"defaults": {"mascotMood": False, "mascotMoodModelPreset": "cheap"}}
    })
    snake = Config.model_validate({
        "agents": {"defaults": {"mascot_mood": False, "mascot_mood_model_preset": "cheap"}}
    })
...
def test_mascot_mood_serializes_in_camel_case():
    ...
    assert dumped["mascotMood"] is False
